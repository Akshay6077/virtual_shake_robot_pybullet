...
combine_data_path = '/home/akshay/ros2_ws/src/virtual_shake_robot_pybullet/data/combined_data.pkl'  # Path to combined data
excel_file_path = '/home/akshay/ros2_ws/src/virtual_shake_robot_pybullet/data/Results_Granite.xlsx'  # Path to Excel file
        ...
        if pga_values[i] is not None and pgv_to_pga_values[i] is not None:
            if topple_status[i] == 1:  # Overturn (light gray)
                ...
                if not no_overturn_plotted:  # Plot label only once
                    plt.scatter(pga_values[i], pgv_to_pga_values[i], color='darkgray', marker='o', s=30, alpha=0.8, label='No Overturn')
                    no_overturn_plotted = True
                else:
                    plt.scatter(pga_values[i], pgv_to_pga_values[i], color='darkgray', marker='o', s=30, alpha=0.8)
